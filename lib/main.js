...
function kruskal() {
	...
	if ( opts.groups ) {
		...
		if ( x.length !== opts.groups.length ) {
			throw new RangeError( format( '1Lb9p,IC', 'opts.groups' ) );
		}
		...
		if ( ngroups < 2 ) {
			throw new Error( format( '1Lb8W,GY', 'groups', levels ) );
		}
		...
		groupsIndicators = [];
		if ( ngroups < 2 ) {
			throw new Error( format( '1Lb9q,ID', arg ) );
		}
		for ( i = 0; i < ngroups; i++ ) {
			arg = arguments[ i ];
			if ( !isCollection( arg ) ) {
				throw new TypeError( format( '1Lb8X,GZ', arg ) );
			}
			if ( arg.length === 0 ) {
				throw new Error( format( '1Lb8Y,Ga', arg ) );
			} else {
				n[ i ] = arg.length;
			}
			groupRankSums[ i ] = 0;
	...
	if ( alpha < 0.0 || alpha > 1.0 ) {
		throw new RangeError( format( '1Lb9r,IE', 'alpha', alpha ) );
	}
